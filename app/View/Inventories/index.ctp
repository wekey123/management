...
<div id="page-wrapper">
   ...
   <div class="row">
	...
	<tbody>
		<?php 
        $total_Quantity = '';$total_purchase_price = '';$total_sale_price = ''; 
        $listAvailable = true;
        ...
        <td><?php echo '$'.$inventory['Inventory']['shipping']; ?>&nbsp;</td> 
       <td><?php echo  date('m-d-Y', strtotime($inventory['Inventory']['created']));?>&nbsp;</td>
		...
		<?php  
		 ...
		 if($listAvailable){ 
		 ?>
		 <tr><td><b>Total: </b></td><td><?php echo $total_Quantity;?>&nbsp;</td><?php if($types != 'fulfillment'){  ?><td><?php echo '$'.sprintf("%01.2f",$total_purchase_price);?>&nbsp;</td><?php }  if($types != 'order'){  ?><td><?php echo '$'.sprintf("%01.2f",$total_sale_price);?>&nbsp;</td><?php } ?><td <?php  echo ($types == 'sale') ? 'colspan="4"' : 'colspan="3"'; ?>>&nbsp;</td></tr>
        <?php } ?>
	</tbody>
	</table>
